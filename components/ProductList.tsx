...
export const ProductCard: React.FC<{ product: Product; onClick?: () => void }> = ({ product, onClick }) => (
  <div 
    onClick={onClick}
    className="group flex flex-col items-center bg-white p-4 rounded-xl hover:shadow-xl transition-all duration-300 cursor-pointer border border-transparent hover:border-gray-100"
  >
    <div className="relative w-full aspect-square mb-4 overflow-hidden rounded-lg bg-gray-50">
      ...
      <div className="absolute inset-0 bg-black/0 group-hover:bg-black/5 transition-colors flex items-center justify-center">
         <button className="opacity-0 group-hover:opacity-100 translate-y-4 group-hover:translate-y-0 transition-all duration-300 bg-brand-orange text-white px-6 py-2 rounded-full font-bold text-sm shadow-lg">
            Ver Detalle
         </button>
      </div>
    </div>
...
const ProductList: React.FC<ProductListProps> = ({ products = PRODUCTS, title, onProductClick }) => {
  return (
